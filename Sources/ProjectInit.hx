...
class ProjectInit {
    ...
    static public function run(p_path:String,type:foundry.data.Project.Type,p_project:String ="") {
        path = p_path;
        if(type == Type.twoD){
            project = p_project != "" ? p_project: "Coin Project";
            generateProject2d();
        }
        else{
    ...
    static function generateProject2d(){
        if(!FileSystem.exists(path+"/khafile.js")){
            ...
            + 'await project.addProject(\'$cwd/coin\');\n'
        ...
        if(!FileSystem.exists(path+"/Sources/bcoin")) FileSystem.createDirectory(path+"/Sources/bcoin");

        #if kha_krom
        #else
        kha.Assets.loadBlobFromPath(Main.cwd+"/pjml.coin", function(blob:kha.Blob){
